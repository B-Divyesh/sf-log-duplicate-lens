...
On first reveal, trace lines resolve in a staggered 240ms fade and the ratio needle settles using transform/opacity only. Result numbers cross-fade over 180ms. Buttons move by 1px while pressed, like panel keys. Nothing loops. Under `prefers-reduced-motion: reduce`, trace and needle motion is removed and states update instantly.
...
- `site/public/apple-touch-icon.png` and `site/public/social-card.png`: locally composed on 2026-08-28 from the original Lens cutaway with ImageMagick. The touch icon adds the product’s amber lens mark; the 1200×630 social card pairs the cutaway with the same paper, ink, amber, and teal instrument-panel palette. They introduce no third-party source material.
- `site/public/cli-demo.svg`: hand-authored terminal capture made on 2026-08-28 from `cargo run --quiet -p log-duplicate-lens -- demo` against sample SHA-256 `644e6d5aaa653e4b9a7dfc383fa4d74bee4d539d287c4dcddc29dce926e47233`. The four captured output lines are revealed once; reduced-motion users see the final frame. The adjacent HTML transcript is the accessible fallback. No third-party player or asset is used.
